...
O processo que ocorre durante a implementação do Outbox **leva um pouco mais a sério a mediação da publicação do evento dentro da abordagem transacional** do que o proposto no Dual-Write, e aposta o sucesso da propagação do evento na simplicidade, **por mais que dependa de um processo adicional para ler e publicar**. 

Essa abordagem, por mais que tenha alguns graus de segurança adicionais na garantia de publicação, **requer um custo compucional adicional na base de dados por conta de leitura e escrita constante** na tabela de outbox. Esse cenário **pode se tornar um gargalo em caso de escala de uso**. Entendemos que as capacidades de **leitura e escrita em concorrência podem ser comprometidos e afetar com mais facilidade a performance do sistema** como um todo nesse tipo de cenário transacional, que tende a ser a parte mais difícil de se escalar em sistemas de grande escala.

<br>

A implementação do CQRS pode variar e se complementar entre os cenários apresentados aqui como possibilidades de solução. Assim como diversos patterns apresentados até aqui, o real valor é entender o processo, as garantias e pontos fortes de cada um deles para adaptarmos para desafios do dia a dia nos times de engenharia. 
