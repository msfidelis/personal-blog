...
A consistência eventual nesse tipo de cenário tende a se tornar **mais complexa conforme aumenta a quantidade de fontes de eventos a serem tratadas e resumidas**. Vamos expandir o exemplo do sistema hospitalar, onde agora precisamos recuperar todo o histórico do paciente para auditoria, faturamento e treinamento de modelos.

Nesse sistema, temos serviços espalhados pela arquitetura responsáveis pela triagem inicial, prescrições médicas, exames laboratoriais e exames de imagem. Precisamos consolidar essas informações por atendimento médico individual e também recuperar todo o histórico do paciente ao longo dos anos. Esse é um caso interessante para usar uma visualização consolidada entre vários domínios que expõem seus dados por meio de tópicos de consolidação. Podemos criar listeners tanto para eventos de comando quanto para tópicos de resposta que confirmam o sucesso da execução, construindo um modelo de consulta que agrupa os dados.

Esse modelo de leitura deve permitir atualizações incrementais e aceitar uma consistência eventual constante, já que o tempo de construção do registro pode variar conforme a demanda e o número de fontes de dados. **Essa abordagem é útil para recuperar dados online, mesmo com consistência eventual**. É uma alternativa a jobs de ETL que fazem essa agregação por meio de batchs programados, ou a padrões como API-Composition, que podem comprometer a disponibilidade do serviço devido ao alto acoplamento entre diferentes serviços para construir a resposta.
