...
Vamos propor um cenário. Em um backend monolítico, em ambientes modernos, se os clientes precisassem recuperar alguns dados ou consumir serviços dessa aplicação, eles fariam uma chamada de API para a URL do backend, e um balanceador de carga encaminharia a requisição para um dos nodes disponíveis na sua lista de hosts. Até aí, nada novo. 
