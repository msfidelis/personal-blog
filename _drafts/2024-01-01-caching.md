...
Neste artigo vamos abordar conceitos de cacheamento, ou caching, com a ótica de System Design. À medida que exploramos as possibilidades de uso do caching, é fundamental compreender que, embora o conceito subjacente seja universal — armazenar dados temporariamente em algum outro lugar para reduzir o tempo de acesso na fonte original —, a aplicação prática dessa técnica pode variar bastante. Existem várias estratégias e tipos de cache. No inicio desse capítulo você vai notar uma constante evolução. Vamos começar com definições conceituais e generalistas que podem ser reaproveitadas na maioria dessas possibilidades, até começarmos a abordar essas estratégias de formas mais específicas com suas particularidades e vantagens.

Este é o sexto artigo sobre System Design, e esse capitulo em especial foi muito divertido de ser produzido. Espero que seja de grande proveito pra todos. 
...
Como já foi mencionado anteriormente, o cache em si é uma estratégia que visa cumprir uma finalidade, e não se baseia em uma tecnologia em específico. Também vimos que existem vários tipos de implementação de caching possíveis. O objetivo dessa sessão é detalhar alguns dos principais usos e aplicações de caching em vários cenários. 
...
Em sistemas distribuídos, o caching é um grande facilitador para melhoria de performance, reduzir a latência e escalar eficientemente ao servir conteúdo dinâmico ou estático. Diferente do cache em memória que tem o escopo de uma execução, thread ou processo, diferentes tecnologias de cache, como Redis, Memcached, entre outras, permitem a distribuição e paralelização do cache, otimizando o acesso a dados em ambientes altamente distribuídos e de alta demanda, permitindo que várias replicas e sistemas difernetes tenham acesso aos mesmos itens cacheados ao mesmo tempo. 

Esse tipo de aplicação é muito bem vinda em cargas de trabalho que são sensíveis a escalabilidade horizontal e que trabalhem com paralelização externa, uma vez que independende de qualquer uma dessas replicas que crie o cache, o mesmo estará acessível pra as demais consultarem no mesmo instante. 

Muitas das tecnologias de caching distribuído que nos permitem trabalhar com capacidades de cache pragmáticamente, também são desenhadas para ter a capacidade de adicionar mais nós ao cluster de cache para lidar com maior carga sem degradar o desempenho, mecanismos para garantir que os dados sejam replicados entre nós, mantendo a consistência dos dados em todo o sistema, possuem implementações de alta disponibilidades que nos permite continuar operando sem interrupção significativa na presença de falhas de um ou mais nós e condições de particionamento. 
...
As abordagens de **write-through**, ou abordagens de **escrita dupla**, tendem a ser aplicadas onde o **cache é pensado de forma mais durável** e a leitura deve manter um certo padrão no tempo de resposta. Basicamente essa estratégia se consiste em **atualizar a versão mais recente do dado simultâneamente entre a base de dados principal e a camada de cache**, **assim que é o mesmo é inserido ou modificado**. 

![Write Through](/assets/images/system-design/Scale-Cache-Write-Through.drawio.png)

O principal objetivo do Write-Through é garantir que os dados no cache estejam sempre atualizados e sincronizados com os dados persistentes, minimizando o risco de inconsistências. Sistemas implementando essa estratégia devem ter mecanismos robustos de recuperação de falhas para lidar com situações onde a escrita na fonte de dados falha após os dados terem sido escritos no cache, para fins de consistência, é comum que as abordagens de cache-aside e write-through sejam implementadas de forma complementar. 
...
A estratégia de **Write-Behind**, também conhecido como **Lazy Writing**, é uma estratégia de gerenciamento de cache que busca otimizar o **desempenho de escrita em aplicações**, minimizando a latência e reduzindo a carga sobre a fonte de dados persistente. Diferentemente do Write-Through, onde as operações de escrita são imediatamente refletidas tanto no cache quanto na fonte de dados, o Write-Behind **adia a sincronização com a fonte de dados, aproveitando períodos de baixa atividade ou políticas específicas para atualizar os dados persistentes**.

![Write-Behind](/assets/images/system-design/Scale-Cache-Write-Behind.drawio.png)

No Write-Behind, as **operações de escrita são primeiramente aplicadas ao cache**, permitindo que a aplicação continue sua execução sem a necessidade de esperar pela confirmação da fonte de dados persistente. Essas escritas são então, de forma assíncrona, propagadas para a fonte de dados em um momento posterior, baseando-se em critérios predefinidos como intervalos de tempo, quantidade de operações acumuladas, quando o sistema detecta um período de baixa demanda ou através de componentes intermediários como filas e event brokers, necessitando de alguma outra aplicação ou processo em paralelo para realizar a escrita. 
