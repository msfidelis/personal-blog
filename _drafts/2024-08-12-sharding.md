...
## Sharding por MurmurHash

O MurmurHash é uma função de hash rápida, barata e não criptográfica, comumente utilizada para criar identificadores de hash a partir de strings ou outros dados, transformando o mesmo em números inteiros, dando a capacidade nativa de se lidar com ranges e intervalos de valores para realizar distribuição. O murmur é amplamente utilizada em sistemas distribuídos por sua capacidade de distribuir quase que uniformemente os valores de hash entre os nodes de um sistema. Um dos principais objetivos ao utilizar uma função de hash para sharding é garantir que os dados sejam distribuídos uniformemente entre os shards. 

O MurmurHash tem uma excelente capacidade de espalhar os valores de hash de maneira uniforme, minimizando a probabilidade de criar hot partitions mediante a quantidade desbalanceada. Quando você aplica o MurmurHash a uma string, sendo ela sua sharding key, o algoritmo  gera um valor de hash numérico. Esse valor de hash é um número inteiro de 32 ou 64 bits, dependendo da versão do MurmurHash utilizada.
