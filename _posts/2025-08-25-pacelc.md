...
Após uma boa assimilação das classificações **AP, AC, CP e PC** do CAP, podemos aprofundar o entendimento nos **apêndices trazidos pelo PACELC**.
...
Normalmente, esses sistemas contam com algoritmos complexos de [**CRDTs**](/replicacao/) (*Conflict-Free Replicated Data Types*), que fazem a gestão de conflitos entre diferentes atualizações de dados em nós distribuídos. Esse modelo é menos comum, mas pode aparecer em **contextos híbridos de microserviços**, nos quais a experiência do usuário não pode parar mesmo com falhas parciais, mas em que a **regra de negócio e a criticidade operacional exigem que, quando a rede está sau**
